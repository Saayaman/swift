...
func wordN() -> Character {
//    let lastChar = word.characters.last
//    return lastChar!
   return word[word.endIndex]
}

//もしくは
//var removed = word.remove(at: word.index(before: word.endIndex))
...
func combineLastCharacters(_ nonsenseArray: [String]) -> String{
    
    for collect in nonsenseArray{
        newstr.append(collect[collect.endIndex])
    }
        return newstr
}
//: __Problem 3__
...
var prices =  ["123","as", "123"]
var boo = false;
func calExpense(price: [String]) -> Bool{
    for price in prices{
//    let numberCharacters = price.rangeOfCharacterFromSet(NSCharacterSet.decimalDigitCharacterSet().invertedSet)
    let numberCharacters = NSCharacterSet.decimalDigits.inverted
    if price.rangeOfCharacter(from: numberCharacters) != nil{
        boo = true
    } else {
...
func cleanUp(dWordsArray: [String]) -> [String]{
    
    var cleanArray = [String]()
    for word in dWordsArray{
        
        if word.characters.count == 4{
//            dWordsArray.remove("word")
        } else{
...
cleanUp(dWordsArray: dirtyWordsArray)
...
class MovieArchive {
    ...
    func filterByDirector(director: String, movies: Dictionary<String, String>) -> Array<String>{
        
        var mymovies = [String]()
        for (director, movie) in movies{
...
myArchive.filterByDirector(director:"Inception", movies:movies)
