...
var filterMicrons = particleSizesInMicrons.filter({(microns:Int) -> Bool in
    microns < 20
})
...
let mappedRupee = pricesInRupees.map({"$\(roundf(Float($0)))"})
print(mappedRupee)
...
let newTimes = oldTimes.map({ timeStringFromInterval(timeIntervalFromString($0)+60)})
print(newTimes)
