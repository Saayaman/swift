...
for rent in oldMountainViewRent{
    newMountainViewRent.append(rent * 0.2)
}
...
for (name,phone) in rapperPhoneNumbers{
    if (phone.range(of: "(415)") != nil){
        var replace = phone.replacingOccurrences(of: "(415)", with: "(510)")
//        print(replace)
        rapperPhoneNumbers[name] = replace
    }
